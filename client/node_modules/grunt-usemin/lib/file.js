    ...
    if (block && last) {
      ...
      if (asset && asset[2]) {
        ...
        if (defer && last.defer === false || last.defer && !defer) {
          throw new Error('Error: You are not suppose to mix deferred and non-deferred src in one block.');
        } else if (defer) {
          last.defer = true;
        } else {
          last.defer = false;
        ...
        if (async && last.async === false || last.async && !async) {
          throw new Error('Error: You are not suppose to mix asynced and non-asynced src in one block.');
        } else if (async) {
          last.async = true;
        } else {
          last.async = false;
